...
- **▶ 播放游戏**：点游戏 →「播放游戏」→ 内置 WKWebView 运行 MV/MZ，带**运行时翻译**（自动加载 `translations/` 词典，离线命中）＋**作弊器**（金钱/满级/全道具/不遇敌/无敌/穿墙/一击必杀/战斗胜利·逃跑/伤害倍率/速度）＋**存档管理**（备份/恢复 localStorage，对应 mtool 存档功能）
- **批量翻译**：把 `www/data` 的地图事件文本和数据库文本批量提取、翻译、写回，导出整份中文游戏
