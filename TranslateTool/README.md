...
1. **导入游戏**：把含 `www/data`（或 `data`）的游戏文件夹放进 App 文稿目录（文件App → 我的 iPhone → RPG 翻译器；或爱思助手直拖 Documents）；含 `[ ]` 空格等特殊字符的文件夹名会在播放时自动改安全名，不影响显示。**首次启动已自动建好 `translations/`、`Saves/`、`Exports/`、`Backups/` 目录和「使用说明.txt」**，拖文件时直接可见
2. **首次打开游戏会弹「允许本地网络」**：点允许（本机 127.0.0.1 回环加载游戏，不访问局域网设备）；误点拒绝可在 系统设置 → 隐私 → 本地网络 重新打开
