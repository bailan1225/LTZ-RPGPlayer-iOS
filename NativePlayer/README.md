...
| 目标语言 | 默认 `zh-CN` |
| 自定义API地址 | 把 `{text}` 作为原文占位符、`{key}` 作为 API Key 占位符、`{prompt}` 作为提示词占位符（对应 AiNiee 提示词优化）；返回 JSON 的 `translatedText` / `translation` 字段或纯文本 |
| API Key | 自定义接口的密钥（旧的过期了直接在这里重新填） |
| 翻译提示词 | 附加到自定义 API 请求的提示词（可留空） |
| 清除翻译缓存 | 下次打开游戏生效 |

- **离线词典**：把 `dict.json`（键=原文，值=译文，UTF-8）放入「文件」App 的 RPG Player 文件夹，App 会优先读取；内置样例含常用日文 RPG 词汇。
- **外部翻译文件（mtool 兼容）**：把 mtool 或其他工具导出的 `{原文: 译文}` JSON 放进 `translations/` 文件夹（文件App → 我的 iPhone → RPG Player → translations），运行时直接命中——离线、即时、术语一致，与翻译器 App 导出的映射格式互通。
- **翻译状态**：游戏列表会显示每个游戏「已翻译 N 条（播放时自动命中）」——由翻译器生成 `translations/<目录名>.json` 后自动出现。
